...
def submitter_handler():
    ...
    if sys.platform not in ("linux", "linux2"):
        sys.exit("Invalid OS: background Submitter launching works only on " "Linux")

    parser = ap.ArgumentParser(
        description="Use this script to start or stop "
    ...
    try:
        ...
        sys.exit(
            "Invalid submitter_file argument: file is not a valid Python" " module"
        )
    ...
    subms = {
        v: getattr(loaded_module, v)
        for v in dir(loaded_module)
        if v[:2] != "__" and issubclass(getattr(loaded_module, v).__class__, Submitter)
    }
    ...
    if args.action[0] == "start":
        ...
        for s in subm_l:
            if os.path.abspath(s[0]) == abs_subm_file and submitter_name == s[1]:
                sys.exit("The requested submitter is already" " running")
        cmd = [
            "python",
            "-m",
            "soprano.hpc.submitter._spawn",
            abs_subm_file,
            submitter_name,
            "&",
        ]
        ...
        # PKILL the process
        succ = Submitter.stop(abs_subm_file, submitter_name)
        if succ:
            print(
                "Submitter "
        ...
        subm_l = Submitter.list()
        for s in subm_l:
            if os.path.abspath(s[0]) == abs_subm_file and s[1] == submitter_name:
                os.kill(s[3], signum)
                break
    else:
