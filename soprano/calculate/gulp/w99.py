...
def find_w99_atomtypes(s, force_recalc=False):
    ...
    if not set(chsyms).issubset(set(["C", "H", "O", "N"])):
        raise W99Error(
            "Invalid system for use of W99 force field"
    ...
    for m_i, m in enumerate(mols):
        ...
        for a_mi, a_i in enumerate(inds):
            ...
            if len(bnd_i) < 1:
                # Something's wrong
                raise W99Error(
                    "ERROR - W99 can not be applied to single"
                    " atom molecular fragments"
                )
            if el_i == "H":
                # Hydrogen case
                if len(bnd_i) != 1:
                    raise W99Error("ERROR - Hydrogen with non-1 valence " "found")
                ...
                if nn_el == "C":
                    ...
                    oxys = [
                        b for b in bonds[c_mi] if chsyms[b] == "O" and b != bnd_i[0]
                    ]
                    # What is it?
