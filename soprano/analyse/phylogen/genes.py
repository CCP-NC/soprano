...
def parsegene_mol_rot(c, Z=0, twist_axis=None, swing_plane=None):
    molr = MoleculeRelativeRotation(
        size=int(Z * (Z - 1) / 2), twist_axis=twist_axis, swing_plane=swing_plane
    )
...
def parsegene_hbonds_fprint(c):
    hblen = HydrogenBondsNumber.get(c)
    return np.array([zip(*sorted(zip(ld.keys(), ld.values())))[1] for ld in hblen])
...
def parsegene_hbonds_length(c):
    ...
    for hbn in hblist:
        ...
        hblen = [
            sorted(cap_to([hb["length"] for hb in hbn[hbs]], hbnmax[hbs]))
            for hbs in hbn
        ]
...
def parsegene_hbonds_angle(c):
    ...
    for hbn in hblist:
        # Extract lengths, order by key
        hblen = [
            sorted(cap_to([hb["angle"] for hb in hbn[hbs]], hbnmax[hbs])) for hbs in hbn
        ]
        hblen = list(itertools.chain(*hblen))
        hblens.append(hblen)
...
def parsegene_coord_histogram(c, s1="C", s2="H", max_coord=6):
    ...
    chist = CoordinationHistogram(species_1=s1, species_2=s2, max_coord=max_coord)
...
def parsegene_bond_order(
    c, s1=None, s2=None, channels=10, cutoff_radius=2.0, cutoff_width=0.05, mode="Q"
):
...
def parsegene_defect_asymmetric_fdist(c, index=0, struct=None):
    ...
    if struct is None:
        raise ValueError("defect_asymmetric_fdist gene requires a struct " "argument")
...
def parsegene_defect_asymmetric_fpos(c, index=0, struct=None):
    ...
    fp = c.all.get_scaled_positions()[:, index, :]
    _, imgs = compute_asymmetric_distmat(struct, fp, return_images=True)
...
class GeneDictionary:
    ...
    _gene_dictionary = {
        "energy": {"default_params": {}, "parser": parsegene_energy, "pair": False},
        "latt_cart": {
            "default_params": {},
        ...
        "coord_histogram": {
            "default_params": {"s1": "C", "s2": "H", "max_coord": 6},
