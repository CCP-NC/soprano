...
def genomes2png(aC, pC, outf, dpi=90):
    ...
    plt.subplots_adjust(left=150.0/figw, right=1.0-50.0/figw, bottom=50.0/figh, top=1.0-50.0/figh)
...
#parser.add_argument('-Z', type=int, default=None, help="Expected numbers of molecules in cell (discard structures that don't agree)")
...
if args.genomes:
    with open(args.seedname + "_genomes.dat", 'w') as ofile:
        ...
        # Create an header
        header = ('Structure\t'
                  '{0}\n').format('\t'.join(['\t'.join(['{0}_{1}'.format(l[0],
                                                                         i+1)
                                                        for i in range(l[1])])
                                             for l in legend]))
        ofile.write(header)
        try:
            ...
            # Someone has no name!
            raise RuntimeError('Not all structures have a name, can\'t print'
                               ' out genomes as table')
...
"""
if args.distmat:
    aRC.create_distance(gene_list)
    with open(args.seedname + "_distmat.json", 'w') as ofile:
        json.dump(distmat2json(aRC), ofile)

if args.forcemat:
    aRC.create_distance(gene_list)
    with open(args.seedname + "_forcemat.json", 'w') as ofile:
        json.dump(forcemat2json(aRC), ofile)
"""
